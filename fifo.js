...
const tableContainer = document.getElementById("table-fifo-container");
...
salesForm.addEventListener("submit", (e) => {
  ...
  if (salesQuantity > 0) {
    // Process sales with FIFO method
...
// Function to process sales with FIFO method and return sale entries for the log
function processSale(salesQuantity, date) {
  let remainingSalesQuantity = salesQuantity;
  let saleEntries = [];

  // Process sales in FIFO order
  for (let i = 0; i < inventory.length && remainingSalesQuantity > 0; i++) {
...
// Function to render FIFO table with the new column order
